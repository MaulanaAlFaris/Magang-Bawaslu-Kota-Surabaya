...
class KecamatanTandesController extends Controller
{
    ...
    public function show(string $id)
    {
        ...
        return view('content.kecamatan.Kecamatan_Tandes.show')->with('data', $data);
    }
    ...
    public function pdf(string $id, Repository $config, Filesystem $files, Factory $view, Request $request)
    {
        ...
        $pdf = Pdf::loadView('content.kecamatan.Kecamatan_Tandes.pdf', ['data' => $data]);

        if ($request->input('download')) {
            return $pdf->download("KecamatanTandes_{$id}.pdf");
        } else {
            return $pdf->stream();
        }
    }

    public function searchTandes(Request $request)
    {
        $searchKeyword = $request->input('keyword');

    if ($searchKeyword) {
        $data = Kecamatan_Tandes_Form::where('nama_pelaksana', 'LIKE', "%$searchKeyword%")
        ...
        if ($data->isEmpty()) {
            return view('content.kecamatan.Kecamatan_Tandes.index')
                ->with('data', $data)
